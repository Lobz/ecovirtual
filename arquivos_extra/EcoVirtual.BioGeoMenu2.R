...
n.islVar <- tclVar("10")
n.islEntry <- tkentry(top, width = "3", textvariable = n.islVar)
seed.rainVar <- tclVar("100")
seed.rainEntry <- tkentry(top, width = "4", textvariable = seed.rainVar)
ar.minVar <- tclVar("10")
...
SVar <- tclVar("100")
SEntry <- tkscale(top, from=10, to=10000, showvalue=TRUE, variable=SVar, resolution=10, orient="horizontal", command=set.gr.abund)
abundVar <- tclVar("0.00")
####
...
abundEntry <-tkscale(top, from=0, to=1, showvalue=TRUE, variable=abundVar, resolution=0.01, orient="horizontal",command=set.gr.abund)
animaVar <- tclVar("1")
...
onOK <- function() 
	...
	n.isl=as.numeric(tclvalue(n.islVar))
	seed.rain=as.numeric(tclvalue(seed.rainVar))
	...
	abund <- as.numeric(tclvalue(abundVar))
	rank <- 1:S
	abund <- abund*(1-abund)^(rank-1)
	animaVF <- as.logical(as.numeric(tclvalue(animaVar)))
   dsnameValue <- trim.blanks(tclvalue(dsname))
        if (dsnameValue == "Do_Not_Save" | dsnameValue=="") 
        {
        	command <- paste("arquip(n.isl= ", n.isl, ",ar.min= ", ar.min,",ar.max= ",ar.max, ", S= ",S, ", seed.rain = ", seed.rain,", abund = ", abund,", tmax = ", tmax,", anima = ", animaVF,")" , sep = "")
        }
        else  
		  {
		  command <- paste(dsnameValue, "<- arquip(n.isl= ", n.isl, ",ar.min= ", ar.min,",ar.max= ",ar.max, ", S= ",S, ", seed.rain= ", seed.rain,", abund = ", abund,", tmax = ", tmax,", anima = ", animaVF,")" , sep = "")
		  }
	doItAndPrint(command)
...
tkgrid(tklabel(top, text = "Number of Islands"), n.islEntry, sticky = "e")
...
tkgrid(tklabel(top, text = "Number of species "), SEntry, sticky = "se")
tkgrid(tklabel(top, text = "Abundance eveness "), abundEntry, sticky = "se")
tkgrid(tklabel(top, text = "Seed rain size "), seed.rainEntry, sticky = "se")
tkgrid(tklabel(top, text = "Show simulation frames"), animaBox, sticky = "e")
#
tkgrid(buttonsFrame, sticky = "w", columnspan = 2)
tkgrid.configure(entryDsname, sticky = "w")
tkgrid.configure(tmaxEntry, sticky = "w")
tkgrid.configure(n.islEntry, sticky = "w")
tkgrid.configure(ar.minEntry, sticky = "w")
tkgrid.configure(ar.maxEntry, sticky = "w")
#
tkgrid.configure(SEntry, sticky = "w")
tkgrid.configure(abundEntry, sticky = "w")
tkgrid.configure(seed.rainEntry, sticky = "w")
#
...
ColExtDbox<-function () 
{
#animaColExt(min=0.01, max=1, cicles=0.01, Ext="crs", Col="dcr")
require(EcoVirtual)
